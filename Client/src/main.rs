...
fn get_mac_address() -> Result<String, Box<dyn std::error::Error>> {
    #[cfg(target_os = "linux")]
    {
        let output = Command::new("cat")
            ...
            .or_else(|_| {
                Command::new("cat")
                    .arg("/sys/class/net/wlan0/address")
                    .output()
            })?;
    ...
    Ok("00:11:22:33:44:55".to_string()) // Fallback for testing
}
...
async fn get_folders() -> Result<HttpResponse> {
    ...
    let mac_address = match get_mac_address() {
        Ok(mac) => {
            println!("🔍 Detected MAC address: {}", mac);
            mac
        }
        Err(e) => {
...
async fn main() -> std::io::Result<()> {
    ...
    match get_mac_address() {
        Ok(mac) => println!("🔍 Detected MAC Address: {}", mac),
    ...
    println!("🔗 Client authenticating with server using MAC address");
